# Tree-map TreeMap 樹狀圖

層次化樹狀圖組件，適合顯示層次結構和比例關係
...
```tsx
import { Tree-map } from '@d3-components/statistical';
...
| 屬性 | 類型 | 預設值 | 說明 |
|------|------|--------|------|
| `aria-label` | `string` | `'TreeMap 樹狀圖'` | ARIA 標籤 |
...
### 繼承架構
```
Tree-map (React Component)
└── D3Tree-map (D3 Logic Class)
