# Candlestick-chart CandlestickChart K線圖

專業股市 K 線圖組件，適合金融數據分析和技術分析
...
```tsx
import { Candlestick-chart } from '@d3-components/financial';
...
| 屬性 | 類型 | 預設值 | 說明 |
|------|------|--------|------|
| `aria-label` | `string` | `'CandlestickChart K線圖'` | ARIA 標籤 |
...
### 繼承架構
```
Candlestick-chart (React Component)
└── D3Candlestick-chart (D3 Logic Class)
